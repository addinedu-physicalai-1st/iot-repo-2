# ESP32-CAM 서버 666 — 양방향 REST API 제어 + UDP 영상 수신(별도 스레드)
# REST API: HTTP 7080 (장비 등록, 명령 폴링, UI에서 캠/플래시 명령)
# UDP 7070: 영상 수신 전용 스레드
...
_api_state = {
    ...
    "server_for_device": {"server_host": "192.168.25.35", "rest_port": 7080, "udp_port": 7070},
    "lock": threading.Lock(),
}
...
def _get_first_device_ip():
    """등록된 장비 목록에서 IP가 있는 첫 장비의 IP를 반환."""
    with _api_state["lock"]:
        for d in _api_state["devices"]:
...
def _send_command_to_esp(cmd: str, log_callback):
    """ESP32 WebServer (/api/device/command) 로 직접 REST 명령 전송."""
    if not _requests_available:
        if log_callback:
            log_callback("⚠ requests 미설치: pip install requests 후 ESP 명령 전송 가능")
        return
    ip = _get_first_device_ip()
    if not ip:
        if log_callback:
            log_callback("⚠ 등록된 장비 IP가 없습니다. 먼저 ESP가 /api/device/register 를 호출해야 합니다.")
        return
    url = f"http://{ip}/api/device/command"
    try:
        # 쿼리스트링 command 로 전달하면 ESP WebServer 가 간단히 파싱 가능
        resp = requests.get(url, params={"command": cmd}, timeout=2)
        if log_callback:
            log_callback(f"📡 ESP({ip}) ← {cmd} → HTTP {resp.status_code}")
    except Exception as e:
        if log_callback:
            log_callback(f"⚠ ESP 명령 전송 실패: {e}")
...
def api_register():
    """
    ESP32 클라이언트가 초기 등록할 때 호출.
    - body.ip 가 있으면 우선 사용
    - 없으면 HTTP remote_addr 로 IP 추정
    """
    try:
        j = request.get_json(force=True, silent=True) or {}
        guid = (j.get("guid") or "").strip() or "ESP32-CAM-01"
        name = (j.get("name") or "").strip() or "CamStream"
        ip_body = (j.get("ip") or "").strip()
        ip_remote = request.remote_addr or ""
        ip_final = ip_body or ip_remote
        _register_device(guid, name, ip_final or None)
        return jsonify({"ok": True, "guid": guid, "ip": ip_final, "remote_ip": ip_remote})
    except Exception as e:
...
def api_device_config():
    ...
    try:
        ...
        rest_port = int(j.get("rest_port", 7080))
        udp_port = int(j.get("udp_port", 7070))
...
class CamServerWorker555(QObject):
    ...
    def start(self):
        ...
        # 3) 주기적으로 장비 목록 시그널 (등록된 장비가 있으면 UI에 전달) + ESP keep-alive(/api/devices/alive) 호출
        last_alive_ping = 0.0
        while True:
            time.sleep(1.0)
            devs = _get_devices()
            ...
            # 10초마다 한 번씩 /api/devices/alive 호출 → ESP 쪽 lastAliveHit 갱신
            now = time.monotonic()
            if _requests_available and devs and now - last_alive_ping >= 10.0:
                last_alive_ping = now
                for d in devs:
                    ip = (d.get("ip") or "").strip()
                    if not ip:
                        continue
                    try:
                        url = f"http://{ip}/api/devices/alive"
                        resp = requests.get(url, timeout=2)
                        self.log_signal.emit(f"[KEEPALIVE] {ip} → /api/devices/alive HTTP {resp.status_code}")
                    except Exception as e:
                        self.log_signal.emit(f"[KEEPALIVE] {ip} 실패: {e}")
...
class CamApp555(QWidget):
    ...
    def initUI(self):
        layout = QVBoxLayout()
        layout.addWidget(QLabel("ESP32-CAM 서버 666 (REST API 7080, UDP 7070)"))
        ...
        self.edit_server_host.setPlaceholderText("192.168.0.149")
        self.edit_server_host.setText("192.168.0.149")
        grid.addWidget(self.edit_server_host, 0, 1)
        grid.addWidget(QLabel("REST 포트:"), 1, 0)
        self.edit_rest_port = QLineEdit()
        self.edit_rest_port.setPlaceholderText("7080")
        self.edit_rest_port.setText("7080")
        grid.addWidget(self.edit_rest_port, 1, 1)
        grid.addWidget(QLabel("UDP 포트:"), 2, 0)
        self.edit_udp_port = QLineEdit()
        self.edit_udp_port.setPlaceholderText("7070")
        self.edit_udp_port.setText("7070")
    ...
    def _api_cam_start(self):
        _set_command("cam_start")
        _send_command_to_esp("cam_start", self.log_append)
        self.log_append("📡 [명령] 캠 영상 전송 시작 (ESP32에 REST 전송)")

    def _api_cam_stop(self):
        _set_command("cam_stop")
        _send_command_to_esp("cam_stop", self.log_append)
        self.log_append("📡 [명령] 캠 영상 전송 중지 (ESP32에 REST 전송)")

    def _api_flash_on(self):
        _set_command("flash_on")
        _send_command_to_esp("flash_on", self.log_append)
        self.log_append("📡 [명령] 플래시 ON (ESP32에 REST 전송)")

    def _api_flash_off(self):
        _set_command("flash_off")
        _send_command_to_esp("flash_off", self.log_append)
        self.log_append("📡 [명령] 플래시 OFF (ESP32에 REST 전송)")

    def _api_flash_blink(self):
        _set_command("flash_blink")
        _send_command_to_esp("flash_blink", self.log_append)
        self.log_append("📡 [명령] 플래시 깜빡임(3회) (ESP32에 REST 전송)")
    ...
    def _refresh_server_edits_from_state(self):
        ...
        self.edit_rest_port.setText(str(cfg.get("rest_port", 7080)))
        self.edit_udp_port.setText(str(cfg.get("udp_port", 7070)))

    def _save_server_config(self):
        host = self.edit_server_host.text().strip() or "192.168.25.35"
        try:
            rp = int(self.edit_rest_port.text().strip() or "7080")
            up = int(self.edit_udp_port.text().strip() or "7070")
        except ValueError:
