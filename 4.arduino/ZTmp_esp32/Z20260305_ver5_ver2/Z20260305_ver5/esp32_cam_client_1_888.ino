// ESP32-CAM 클라이언트 888 — 기동 후 자동 UDP 스트리밍 (REST 명령 없음)
// - WiFi 연결 후 즉시 UDP 영상 전송 시작 (cam_start 불필요)
// - REST API(WebServer) 기동은 주석 처리 — 필요 시 아래 주석 해제
...
// 888: 기동 후 자동 스트리밍 — true 로 고정 (REST cam_start 불필요)
...
// --- [태스크 1] 웹 서버 (기동 시 스타트하는 부분은 setup() 에서 주석 처리됨) ---
...
// --- [태스크 3] UDP 영상 전송 (기동 후 자동 전송) ---
void taskUdpStream(void* pvParameters) {
    Serial.println("[888] UDP Stream started (auto streaming on boot)");
    for (;;) {
        if (camStreaming) {
            ...
            if (fb) {
                ...
                uint8_t packetNo = 0;
                uint8_t checksum = 0;
                for (size_t i = 0; i < imgSize; i++) checksum += fb->buf[i];
                ...
                while (remaining > 0) {
                    ...
                    udp.write(frameNo);
                    udp.write(packetNo++);
                    udp.write(isLast ? 1 : 0);
                    udp.write(isLast ? checksum : (uint8_t)0);
                    udp.write(p, chunk);
                    udp.endPacket();
                    p += chunk;
                    remaining -= chunk;
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n--- ESP32-CAM client 888 (auto UDP stream, no REST command) ---");
    ...
    // Core 0: UDP 영상 전송 (기동 후 자동 스트리밍)
    xTaskCreatePinnedToCore(taskUdpStream, "UDP", 8192, NULL, 2, NULL, 0);

    // --- REST API 서버 기동: 기동 후 스타트하는 부분 주석 처리 (888은 자동 스트리밍만 사용) ---
    ...
    Serial.println("[888] Init done. UDP streaming auto started.");
}
