# ESP32-CAM 서버 888 — UDP 영상 수신 후 화면 표시 전용
# - REST API 없음. UDP 스트림이 들어오면 자동으로 영상 표시
# - esp32_cam_client_1_888.ino 와 쌍으로 사용 (기동 후 자동 UDP 스트리밍)
...
def udp_receiver_thread(log_signal):
    ...
    last_frame_no = -1
    if log_signal:
        log_signal.emit(f"[888] UDP 수신 시작 (포트 {UDP_PORT}). 스트림 들어오면 영상 표시.")
    while True:
        try:
...
class CamApp888(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("ESP32-CAM 888 — UDP 영상")
        layout = QVBoxLayout()
        ...
        self.video_label.setText("UDP 대기 중... (ESP32 888 기동 후 자동 전송)")
