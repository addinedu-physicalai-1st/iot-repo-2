...
#   - UDP: 7070 (영상 수신 전용, 3.device_client 와 동일 포트)
...
# --- 설정 ---
# 3.device_client/.env 및 esp32_lpr_enter.ino 와 동일 포트 사용
REST_PORT = 7080
UDP_PORT = 7070
...
_api_state = {
    ...
    # 장비에 내려보낼 서버 주소/포트 기본값 (필요하면 UI에서 수정 가능)
    "server_for_device": {"server_host": "192.168.0.149", "rest_port": 7080, "udp_port": 7070},
    "lock": threading.Lock(),
}
...
def api_device_config():
    ...
    try:
        j = request.get_json(force=True, silent=True) or {}
        host = (j.get("server_host") or j.get("host") or "").strip() or "192.168.0.149"
...
class CamApp555(QWidget):
    ...
    def initUI(self):
        layout = QVBoxLayout()
        layout.addWidget(QLabel("ESP32-CAM 서버 555 (REST API 7080, UDP 7070)"))
    ...
    def _refresh_server_edits_from_state(self):
        ...
        self.edit_server_host.setText(cfg.get("server_host", "192.168.0.149"))
    ...
    def _save_server_config(self):
        host = self.edit_server_host.text().strip() or "192.168.0.149"
        try:
