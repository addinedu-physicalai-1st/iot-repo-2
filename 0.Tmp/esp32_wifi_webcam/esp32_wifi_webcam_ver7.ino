...
// 1. 네트워크 및 전송 설정
const char *ssid = "addinedu_201class_2-2.4G"; //wifi 2.4G ssid
const char *password = "201class2!";           //wifi 2.4G ssid_pw
const char* udpAddress = "192.168.0.149";      //전송할 ip
...
void sendImageUDP(uint8_t *imageData, size_t imageSize, uint8_t frameNo)
{
  ...
  while (remainingSize > 0)
  {
    ...
    // 마지막 패킷일 경우 체크섬을 헤더 다음(3번째 바이트)에 추가
    // 일반 패킷은 0을 넣어 자릿수 유지
    if (isLastPacket)
    {
      udp.write(checksum); 
