...
class CamServerWorker(QObject):
    ...
    def _udp_receiver(self):
        """패킷 수집, 체크섬 검증 및 이미지 조립 (cam_udp_receive_test_gui.py udp_receiver와 동일)"""
        ...
        while True:
            try:
                ...
                chunk = data[4:]
                #$self.log_signal.emit(f"[PKT] f_no={f_no} p_no={p_no} is_last={is_last} len={len(chunk)}B from {addr[0]}:{addr[1]}")
