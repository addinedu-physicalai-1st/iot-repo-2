...
def udp_receiver():
    ...
    while True:
        try:
            # 헤더가 3바이트로 늘어남 (f_no, p_no, checksum)
            data, addr = sock.recvfrom(2048)
            if len(data) < 4: continue

            f_no = data[0]
            p_no = data[1]
            received_checksum = data[2] # 아두이노가 보낸 체크섬
            chunk = data[3:]
            ...
            if chunk.find(b'\xff\xd9') != -1:
                ...
                if len(indices) > 0 and indices[0] == 0 and len(indices) == indices[-1] + 1:
                    ...
                    if calculated_checksum == frames[f_no]['target_checksum']:
                        ...
                        print(f"Frame {f_no}: Checksum Error! (Cal:{calculated_checksum} != Recv:{received_checksum})")
