...
#
# [스레드 구조] TCP / UDP 각각 별도 스레드에서 처리
#   - 메인: start() → accept() 루프 (TCP 연결 수락 후 연결당 _handle 스레드 생성)
#   - UDP 전용: _udp_receiver_stable() 1개 스레드 (영상 패킷 수신·조립·frame_queue 적재)
#   - TCP 전용: _handle(conn, addr) → 연결 1개당 1개 스레드 (PING/PONG, 장비목록, CAM_START/STOP 등)
...
class CamServerWorker333(QObject):
    ...
    def _udp_receiver_stable(self):
        """UDP 수신: cam_udp_receive_test_gui.py udp_receiver() 발췌 적용 (패킷 수집·체크섬·이미지 조립)"""
...
class CamApp333(QWidget):
    ...
    def _update_video_frame(self):
        ...
        if img is None:
            return
        # numpy 버퍼 수명 이슈 방지: QImage가 참조만 하므로 복사본으로 표시
