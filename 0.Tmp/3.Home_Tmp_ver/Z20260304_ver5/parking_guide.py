            ...
            while True:
                ...
                if typ == TYPE_IR_EVENT:
                    ...
                    if ev == 99: # 하드웨어 초기화 에러 발생 시
                        line = f"----------------------------------------\n❌ [ERROR] {src} 초기화 실패! ({ext})\n----------------------------------------"
                    elif src.startswith("SPOT_"):
                        PARKING_STATE[src] = ext
                        s1 = "차량있음 🔴" if PARKING_STATE["SPOT_1"] == "OCCUPIED" else "빈자리 🟢"
                        s2 = "차량있음 🔴" if PARKING_STATE["SPOT_2"] == "OCCUPIED" else "빈자리 🟢"
                        s3 = "차량있음 🔴" if PARKING_STATE["SPOT_3"] == "OCCUPIED" else "빈자리 🟢"
                        s4 = "차량있음 🔴" if PARKING_STATE["SPOT_4"] == "OCCUPIED" else "빈자리 🟢"
                        line = (
                            f"----------------------------------------\n"
                            f"🅿️ 4구역 주차 실시간 현황 ({src} 업데이트)\n"
                            f"   [1번] {s1}   |   [2번] {s2}\n"
                            f"   [3번] {s3}   |   [4번] {s4}\n"
                            f"----------------------------------------"
                        )
                        ...
                        event_text = f"[EVENT] {name} | {src}" + (f" | {ext}" if ext else "")
                        line = f"----------------------------------------\n🔔 {event_text}\n----------------------------------------"
...
class IntegratedApp(QWidget):
    ...
    def initUI(self):
        ...
        self.log.setStyleSheet("font-size: 14px; line-height: 1.5;")
